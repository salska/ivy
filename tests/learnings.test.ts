...
describe("queryLearnings", () => {
    test("returns learning events scoped to a project", async () => {
        const { queryLearnings } = await import("../src/learnings");
...
describe("getSteeringRules", () => {
    test("returns only active rules sorted by confidence", async () => {
        const { getSteeringRules } = await import("../src/learnings");
...
describe("synthesizeRules", () => {
    test("creates rules from facts appearing 3+ times", async () => {
        const { synthesizeRules, getSteeringRules } = await import("../src/learnings");
    ...
    test("emits rule_synthesized events", async () => {
        const { synthesizeRules } = await import("../src/learnings");
...
describe("buildPromptContext", () => {
    test("returns formatted prompt string with rules", async () => {
        const { buildPromptContext } = await import("../src/learnings");
        const now = new Date().toISOString();
    ...
    test("increments hit_count on injected rules", async () => {
        const { buildPromptContext, getSteeringRules } = await import("../src/learnings");
        const now = new Date().toISOString();
...
describe("CLI learn", () => {
    test("learn query --project --json returns JSON envelope", async () => {
        ...
        const proc = Bun.spawn(
            ["bun", "src/index.ts", "--db", dbPath, "--json", "learn", "query", "--project", "test-proj"],
        ...
        expect(json.project).toBe("test-proj");
        expect(json.learnings.count).toBeGreaterThanOrEqual(1);
    });

    test("learn analyze --project --json runs synthesis", async () => {
        const proc = Bun.spawn(
            ["bun", "src/index.ts", "--db", dbPath, "--json", "learn", "analyze", "--project", "test-proj"],
            { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
        );
        const text = await new Response(proc.stdout).text();
        await proc.exited;
    ...
    test("learn inject --project outputs prompt block", async () => {
        const proc = Bun.spawn(
            ["bun", "src/index.ts", "--db", dbPath, "learn", "inject", "--project", "test-proj"],
            { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
        );
        const text = await new Response(proc.stdout).text();
        await proc.exited;

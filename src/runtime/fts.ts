...
export function setupFTS5(db: Database): void {
  ...
  // Rebuild FTS index ONLY if events exist but the FTS index is empty.
  // This avoids redundant synchronous rebuilds on every start-up.
  const eventCount = (db.query('SELECT COUNT(*) as count FROM events').get() as { count: number }).count;
  const ftsCount = (db.query('SELECT COUNT(*) as count FROM events_fts').get() as { count: number }).count;

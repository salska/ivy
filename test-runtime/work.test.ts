...
import type {
  CreateWorkItemResult, ClaimWorkItemResult, ReleaseWorkItemResult,
  CompleteWorkItemResult, BlockWorkItemResult, UnblockWorkItemResult,
  DeleteWorkItemResult
} from "../src/kernel/work";
import {
  createWorkItem, claimWorkItem, releaseWorkItem,
  completeWorkItem, blockWorkItem, unblockWorkItem, deleteWorkItem,
  flushActiveWorkItems, flushAllDatabase
} from "../src/kernel/work";
...
describe("createWorkItem", () => {
  test("creates work item with all fields", async () => {
    const { createWorkItem } = await import("../src/kernel/work");
...
describe("claimWorkItem", () => {
  test("claims available work item", async () => {
    const { createWorkItem, claimWorkItem } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
  ...
  test("throws on non-existent item", async () => {
    const { claimWorkItem } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
    const agent = registerAgent(db, { name: "Claimer" });
...
describe("createAndClaimWorkItem", () => {
  test("creates and claims in one transaction", async () => {
    const { createAndClaimWorkItem } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
...
describe("listWorkItems", () => {
  test("returns available items by default", async () => {
    const { createWorkItem, claimWorkItem, listWorkItems } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
  ...
  test("orders by priority ASC then created_at ASC (oldest first)", async () => {
    const { createWorkItem, listWorkItems } = await import("../src/kernel/work");
  ...
  test("filters by project", async () => {
    const { createWorkItem, listWorkItems } = await import("../src/kernel/work");
    const { registerProject } = await import("../src/kernel/project");
  ...
  test("returns empty array for no matches", async () => {
    const { listWorkItems } = await import("../src/kernel/work");
    const items = listWorkItems(db);
...
describe("getWorkItemStatus", () => {
  test("returns item detail with event history", async () => {
    const { createWorkItem, claimWorkItem, getWorkItemStatus } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
  ...
  test("returns empty history for unclaimed item", async () => {
    const { createWorkItem, getWorkItemStatus } = await import("../src/kernel/work");
  ...
  test("throws on non-existent item", async () => {
    const { getWorkItemStatus } = await import("../src/kernel/work");
...
describe("CLI work claim", () => {
  test("create-and-claim outputs JSON", async () => {
    ...
    const regProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "agent", "register", "--name", "CLIAgent"],
    ...
    const claimProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "work", "claim",
...
describe("CLI work list", () => {
  test("list outputs JSON array", async () => {
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "work", "list"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;
  ...
  test("list with --status filter", async () => {
    ...
    const regProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "agent", "register", "--name", "ListAgent"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const regText = await new Response(regProc.stdout).text();
    await regProc.exited;
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "work", "list", "--status", "claimed"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;
...
describe("releaseWorkItem", () => {
  test("releases a claimed item back to available", async () => {
    const { createWorkItem, claimWorkItem, releaseWorkItem } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
  ...
  test("throws on non-existent item", async () => {
    const { releaseWorkItem } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
    const agent = registerAgent(db, { name: "Releaser" });
  ...
  test("throws when item not claimed", async () => {
    const { createWorkItem, releaseWorkItem } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
...
describe("completeWorkItem", () => {
  test("marks a claimed item as completed", async () => {
    const { createWorkItem, claimWorkItem, completeWorkItem } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
...
describe("blockWorkItem", () => {
  test("blocks an available item", async () => {
    const { createWorkItem, blockWorkItem } = await import("../src/kernel/work");
  ...
  test("blocks a claimed item retaining claimed_by", async () => {
    const { createWorkItem, claimWorkItem, blockWorkItem } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
  ...
  test("throws on completed item", async () => {
    const { createWorkItem, claimWorkItem, completeWorkItem, blockWorkItem } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
...
describe("unblockWorkItem", () => {
  test("unblocks to available when no claimed_by", async () => {
    const { createWorkItem, blockWorkItem, unblockWorkItem } = await import("../src/kernel/work");
  ...
  test("unblocks to claimed when claimed_by is set", async () => {
    const { createWorkItem, claimWorkItem, blockWorkItem, unblockWorkItem } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
  ...
  test("throws when item is not blocked", async () => {
    const { createWorkItem, unblockWorkItem } = await import("../src/kernel/work");
...
describe("CLI work release", () => {
  test("release outputs JSON", async () => {
    const regProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "agent", "register", "--name", "CLIReleaser"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const regText = await new Response(regProc.stdout).text();
    await regProc.exited;
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "work", "release",
...
describe("CLI work complete", () => {
  test("complete outputs JSON", async () => {
    const regProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "agent", "register", "--name", "CLICompleter"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const regText = await new Response(regProc.stdout).text();
    await regProc.exited;
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "work", "complete",
...
describe("CLI work block/unblock", () => {
  test("block and unblock output JSON", async () => {
    ...
    const blockProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "work", "block",
    ...
    const unblockProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "work", "unblock",
        "--id", "cli-blk"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
...
describe("deleteWorkItem", () => {
  test("deletes an available work item", async () => {
    const { createWorkItem, deleteWorkItem } = await import("../src/kernel/work");
  ...
  test("deletes a completed work item without --force", async () => {
    const { createWorkItem, claimWorkItem, completeWorkItem, deleteWorkItem } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
  ...
  test("refuses to delete a claimed item without --force", async () => {
    const { createWorkItem, claimWorkItem, deleteWorkItem } = await import("../src/kernel/work");
    const { registerAgent } = await import("../src/kernel/agent");
  ...
  test("deletes a blocked item without --force", async () => {
    const { createWorkItem, blockWorkItem, deleteWorkItem } = await import("../src/kernel/work");
  ...
  test("throws on non-existent item", async () => {
    const { deleteWorkItem } = await import("../src/kernel/work");
  ...
  test("cleans up heartbeat references", async () => {
    const { createWorkItem, claimWorkItem, deleteWorkItem } = await import("../src/kernel/work");
    const { registerAgent, sendHeartbeat } = await import("../src/kernel/agent");
...
describe("CLI work delete", () => {
  test("delete outputs JSON for available item", async () => {
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "work", "delete", "cli-del-1"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;
  ...
  test("delete refuses claimed item without --force", async () => {
    const regProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "agent", "register", "--name", "CLIDelAgent"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const regText = await new Response(regProc.stdout).text();
    await regProc.exited;
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "work", "delete", "cli-del-2"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;
    ...
    expect(err).toContain("claimed");
  });

  test("delete with --force deletes claimed item", async () => {
    const regProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "agent", "register", "--name", "CLIForceAgent"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const regText = await new Response(regProc.stdout).text();
    await regProc.exited;
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "work", "delete", "cli-del-3", "--force"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;
  ...
  test("delete returns error for non-existent item", async () => {
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "work", "delete", "nonexistent"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;
...
describe("updateWorkItemMetadata", () => {
  test("merges new keys into empty metadata", async () => {
    const { createWorkItem, updateWorkItemMetadata } = await import("../src/kernel/work");
  ...
  test("throws on non-existent item", async () => {
    const { updateWorkItemMetadata } = await import("../src/kernel/work");
...
describe("appendWorkItemEvent", () => {
  test("appends a comment_received event", async () => {
    const { createWorkItem, appendWorkItemEvent } = await import("../src/kernel/work");
  ...
  test("throws on non-existent item", async () => {
    const { appendWorkItemEvent } = await import("../src/kernel/work");
  ...
  test("events appear in getWorkItemStatus history", async () => {
    const { createWorkItem, appendWorkItemEvent, getWorkItemStatus } = await import("../src/kernel/work");
...
describe("CLI work status", () => {
  test("status outputs JSON detail", async () => {
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "work", "status", "stat-1"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;
...
describe("flushAllDatabase", () => {
  test("deletes all runtime state and leaves an audit event", async () => {
    const { flushAllDatabase } = await import("../src/kernel/work");

    // Setup dummy data
    db.exec(`
      INSERT INTO projects (project_id, display_name, registered_at) VALUES ('test-proj', 'Test', '2023-01-01T00:00:00Z');
      INSERT INTO agents (session_id, agent_name, status, started_at, last_seen_at) VALUES ('agent-1', 'Agent 1', 'active', '2023-01-01T00:00:00Z', '2023-01-01T00:00:00Z');
      INSERT INTO work_items (item_id, title, source, status, created_at) VALUES ('item-1', 'Test Item', 'system', 'available', '2023-01-01T00:00:00Z');
      INSERT INTO events (event_type, summary, timestamp) VALUES ('test_event', 'Test', '2023-01-01T00:00:00Z');
      INSERT INTO heartbeats (session_id, timestamp) VALUES ('agent-1', '2023-01-01T00:00:00Z');
    `);

    const result = flushAllDatabase(db);
    expect(result.flushed).toBe(true);

    // Verify deletions
    expect((db.query("SELECT COUNT(*) as c FROM agents").get() as { c: number }).c).toBe(0);
    expect((db.query("SELECT COUNT(*) as c FROM work_items").get() as { c: number }).c).toBe(0);
    expect((db.query("SELECT COUNT(*) as c FROM heartbeats").get() as { c: number }).c).toBe(0);

    // Verify projects are kept
    expect((db.query("SELECT COUNT(*) as c FROM projects").get() as { c: number }).c).toBe(1);

    // Verify audit event exists
    const events = db.query("SELECT * FROM events").all() as any[];
    expect(events.length).toBe(1);
    expect(events[0].event_type).toBe('system_flush');
  });
});

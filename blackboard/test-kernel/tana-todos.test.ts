...
describe('evaluateTanaTodos', () => {
  ...
  test('skips resolution when tag_id already looks like a raw ID', async () => {
    let resolveTagCalled = false;
    setTanaAccessor(makeMockAccessor({
      resolveTagId: async () => {
        resolveTagCalled = true;
        return 'should-not-be-used';
      },
      searchTodos: async (opts) => {
        // The raw ID should be passed through directly
        if (opts.tagId === 'ZEYpQorZ9KCT') return [makeTanaNode({ id: 'n1' })];
        return [];
      }
    }));

    const result = await evaluateTanaTodos(makeItem({ config: { tag_id: 'ZEYpQorZ9KCT' } }));
    expect(resolveTagCalled).toBe(false);
    expect(result.status).toBe('alert');
  });

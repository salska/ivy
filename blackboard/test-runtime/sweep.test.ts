...
describe("isPidAlive", () => {
  test("returns false for null pid", async () => {
    const { isPidAlive } = await import("../src/kernel/sweep");
...
describe("sweepStaleAgents", () => {
  test("marks stale agent with dead pid", async () => {
    const { sweepStaleAgents } = await import("../src/kernel/sweep");
...
describe("auto-sweep in createContext", () => {
  test("createContext sweeps stale agents silently", async () => {
    const { registerAgent } = await import("../src/kernel/agent");
    const { createContext, resetContextState } = await import("../src/kernel/context");
...
describe("CLI sweep", () => {
  test("sweep outputs JSON result", async () => {
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "sweep"],
  ...
  test("sweep --dry-run does not modify state", async () => {
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "sweep", "--dry-run"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;
  ...
  test("sweep --threshold overrides config", async () => {
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "sweep", "--threshold", "5"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;
  ...
  test("sweep with no stale agents outputs message", async () => {
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "sweep"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;

    expect(text.trim()).toContain("Swept 0 stale agent(s)");

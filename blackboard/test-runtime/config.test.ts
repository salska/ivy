...
describe("BlackboardConfigSchema", () => {
  test("parses empty object to all defaults", async () => {
    const { BlackboardConfigSchema } = await import("../src/kernel/config");
...
describe("loadConfigFromFile", () => {
  ...
  test("returns empty object when file missing", async () => {
    const { loadConfigFromFile } = await import("../src/kernel/config");
...
describe("applyEnvOverrides", () => {
  ...
  test("applies numeric env overrides", async () => {
    const { applyEnvOverrides } = await import("../src/kernel/config");
...
describe("loadConfig", () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `bb-config-test-${Date.now()}`);
    mkdirSync(tmpDir, { recursive: true });
    const { resetConfigCache } = await import("../src/kernel/config");
    resetConfigCache();
  });
  ...
  test("returns all defaults when no config file", async () => {
    const { loadConfig, resetConfigCache } = await import("../src/kernel/config");
    resetConfigCache();

...
describe("parseDuration", () => {
  test("parses seconds", async () => {
    const { parseDuration } = await import("../src/kernel/events");
...
describe("observeEvents", () => {
  ...
  test("returns events ordered by timestamp DESC by default (most recent first)", async () => {
    const { observeEvents } = await import("../src/kernel/events");
...
describe("formatTimeline", () => {
  test("formats events as timeline lines", async () => {
    const { formatTimeline } = await import("../src/kernel/output");
...
describe("CLI observe", () => {
  test("observe --json returns event envelope", async () => {
    ...
    const regProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "agent", "register", "--name", "Observer"],
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "observe"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
  ...
  test("observe --filter works in JSON mode", async () => {
    // Register an agent
    const regProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "agent", "register", "--name", "Filtered"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    await new Response(regProc.stdout).text();
    await regProc.exited;
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "observe", "--type", "agent_registered"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;
  ...
  test("observe --limit limits results", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      const p = Bun.spawn(
        ["bun", "src/cli.ts", "--db", dbPath, "--json", "agent", "register", "--name", `Limit${i}`],
        { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
      );
    ...
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "observe", "--limit", "2"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;

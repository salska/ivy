...
describe("formatSize", () => {
  test("formats bytes", async () => {
    const { formatSize } = await import("../src/kernel/status");
...
describe("getOverallStatus", () => {
  test("returns correct structure on empty database", async () => {
    const { getOverallStatus } = await import("../src/kernel/status");
...
describe("CLI status", () => {
  test("status --json returns overall status", async () => {
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "status"],
  ...
  test("status human output includes database size", async () => {
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "status"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;

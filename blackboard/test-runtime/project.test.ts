...
describe("listProjects", () => {
  test("returns all projects ordered by registered_at DESC", async () => {
    const { registerProject, listProjects } = await import("../src/kernel/project");
  ...
  test("returns 0 agents for projects with no active agents", async () => {
    const { registerProject, listProjects } = await import("../src/kernel/project");
    const { registerAgent, deregisterAgent } = await import("../src/kernel/agent");
  ...
  test("returns empty array when no projects", async () => {
    const { listProjects } = await import("../src/kernel/project");
    const result = listProjects(db);
...
describe("getProjectStatus", () => {
  test("returns project, agents, and work items", async () => {
    const { registerProject, getProjectStatus } = await import("../src/kernel/project");
    const { registerAgent } = await import("../src/kernel/agent");
  ...
  test("throws PROJECT_NOT_FOUND for missing project", async () => {
    const { getProjectStatus } = await import("../src/kernel/project");
...
describe("getProjectDetail", () => {
  test("returns full project detail with stats", async () => {
    const { registerProject, getProjectDetail } = await import("../src/kernel/project");
    const { registerAgent } = await import("../src/kernel/agent");
  ...
  test("throws PROJECT_NOT_FOUND for missing project", async () => {
    const { getProjectDetail } = await import("../src/kernel/project");
...
describe("CLI project register", () => {
  test("register --id --name outputs project as JSON", async () => {
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "project", "register", "--id", "cli-proj", "--name", "CLI Project", "--path", "/tmp/test"],
...
describe("CLI project status", () => {
  test("status --json returns project with agents and work items", async () => {
    ...
    const regProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "project", "register", "--id", "status-cli", "--name", "CLI Status"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    ...
    const statusProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "project", "status", "status-cli"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
  ...
  test("status for missing project returns error", async () => {
    const proc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "project", "status", "missing-proj"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    const text = await new Response(proc.stdout).text();
    await proc.exited;
    ...
    expect(errText).toContain("Project not found: missing-proj");
  });
});
...
describe("CLI project list", () => {
  test("list --json outputs envelope with items", async () => {
    // Register first
    const regProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "project", "register", "--id", "list-proj", "--name", "List Project"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );
    await new Response(regProc.stdout).text();
    await regProc.exited;
    ...
    const listProc = Bun.spawn(
      ["bun", "src/cli.ts", "--db", dbPath, "--json", "project", "list"],
      { cwd: PROJECT_ROOT, stdout: "pipe", stderr: "pipe" }
    );

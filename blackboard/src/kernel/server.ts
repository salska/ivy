...
import { getOverallStatus } from "./status.ts";
import { getHealthStatus } from "./health.ts";
import { listAgents } from "./agent.ts";
import { listWorkItems, getWorkItemStatus, deleteWorkItem, updateWorkItemMetadata, appendWorkItemEvent, flushActiveWorkItems, approveWorkItem, rejectWorkItem } from "./work.ts";
import { createSnapshot, listSnapshots, restoreSnapshot } from "./snapshot.ts";
import { listProjects, getProjectDetail } from "./project.ts";
import { observeEvents } from "./events.ts";
import { queryLearnings, getSteeringRules } from "./learnings.ts";
import type { BlackboardAgent } from "./types.ts";
import { listSkills } from "../runtime/skills.ts";
import { loadPersona } from "../runtime/scheduler/persona-loader.ts";
  ...
  return Bun.serve({
    ...
    async fetch(req) {
      ...
      try {
        ...
        if (url.pathname === "/api/work/flush" && req.method === "POST") {
          const result = flushActiveWorkItems(db);
        ...
        // Phase 1: Approve/Reject endpoints
        const approveMatch = url.pathname.match(/^\/api\/work\/([^/]+)\/approve$/);
        if (approveMatch && req.method === "POST") {
          const itemId = decodeURIComponent(approveMatch[1]!);
          const result = approveWorkItem(db, itemId);
          return jsonResponse(result, 200, cors);
        }

        const rejectMatch = url.pathname.match(/^\/api\/work\/([^/]+)\/reject$/);
        if (rejectMatch && req.method === "POST") {
          const itemId = decodeURIComponent(rejectMatch[1]!);
          const result = rejectWorkItem(db, itemId);
          return jsonResponse(result, 200, cors);
        }

        // Phase 1: Snapshots endpoints
        if (url.pathname === "/api/snapshots") {
          if (req.method === "GET") {
            const limitStr = url.searchParams.get("limit");
            const limit = limitStr ? parseInt(limitStr, 10) : 20;
            const snaps = listSnapshots(db, limit);
            return jsonResponse({ count: snaps.length, items: snaps }, 200, cors);
          }
          if (req.method === "POST") {
            const body = await req.json() as { trigger?: string };
            const result = createSnapshot(db, body.trigger ?? 'manual');
            return jsonResponse(result, 200, cors);
          }
        }
        ...
        if (url.pathname === "/api/snapshots/restore" && req.method === "POST") {
          ...
          if (!body.snapshot_id) {
            return jsonResponse({ error: "snapshot_id is required" }, 400, cors);
          }
          const result = restoreSnapshot(db, body.snapshot_id);
          return jsonResponse(result, 200, cors);
        }
        ...
        if (url.pathname === "/api/skills") {
          ...
          return jsonResponse(skills, 200, cors);
        }
        ...
        // Personas endpoint — reads agent definitions from ~/.claude/agents/
        if (url.pathname === "/api/personas") {
          ...
          if (existsSync(agentsDir)) {
            ...
            for (const file of files) {
              ...
              const getColor = yaml.match(/^color:\s*(.+)$/m);
              const getBg = yaml.match(/^\s+background:\s*"(.+)"$/m);
              const getTitle = yaml.match(/^\s+title:\s*"(.+)"$/m);
              const getPersonaName = yaml.match(/^\s+name:\s*"(.+)"$/m);

              const name = getName?.[1]?.trim().replace(/^["']|["']$/g, "") ?? file.replace(".md", "");
              const description = getDesc?.[1]?.trim().replace(/^["']|["']$/g, "") ?? "";
              const descLower = description.toLowerCase();
              const dispatchable = !SUB_AGENT_MARKERS.some(m => descLower.includes(m));

              personas.push({
                name,
                description,
                model: getModel?.[1]?.trim() ?? "default",
                color: getColor?.[1]?.trim() ?? "gray",
                persona_name: getPersonaName?.[1]?.trim() ?? null,
                persona_title: getTitle?.[1]?.trim() ?? null,
                background: getBg?.[1]?.trim() ?? null,
                dispatchable,
              });
            }

            // Sort: dispatchable first, then alphabetical
            personas.sort((a, b) => {
          ...
          return jsonResponse({ count: personas.length, items: personas }, 200, cors);
        }
